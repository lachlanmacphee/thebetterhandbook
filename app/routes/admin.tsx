    ...
    if (action === "approve") {
      if (suggestion.field === "name") {
        await db.unit.update({
          where: { id: suggestion.unitId },
          data: { name: suggestion.newValue },
        });
      } else if (suggestion.field === "campus") {
        const [campus, oldCampus] = await Promise.all([
          db.campus.findFirst({
            where: { name: suggestion.newValue },
          }),
          db.campus.findFirst({
            where: { name: suggestion.oldValue },
          }),
        ]);

        if (!campus) {
          return data({ error: "Invalid campus suggestion" }, { status: 400 });
        }

        if (oldCampus) {
          await db.unitCampus.deleteMany({
            where: {
        ...
        await db.unitCampus.create({
          data: {
            unitId: suggestion.unitId,
            campusId: campus.id,
          },
        });
      } else if (suggestion.field === "semester") {
        const [semester, oldSemester] = await Promise.all([
        ...
        if (!semester) {
          return data({ error: "Invalid semester suggestion" }, { status: 400 });
        }

        if (oldSemester) {
          await db.unitSemester.deleteMany({
            where: {
              unitId: suggestion.unitId,
